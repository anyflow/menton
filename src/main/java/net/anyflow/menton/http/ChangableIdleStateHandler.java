...
/**
 * Triggers an {@link IdleStateEvent} when a {@link Channel} has not performed read, write, or both operation for a while. <h3>Supported idle states</h3>
 * <table border="1">
 * <tr>
 * <th>Property</th>
 * <th>Meaning</th>
 ...
 * <td>an {@link IdleStateEvent} whose state is {@link IdleState#READER_IDLE} will be triggered when no read was performed for the specified period of
 * time. Specify {@code 0} to disable.</td>
 * </tr>
 * <tr>
 * <td>{@code writerIdleTime}</td>
 * <td>an {@link IdleStateEvent} whose state is {@link IdleState#WRITER_IDLE} will be triggered when no write was performed for the specified period
 * of time. Specify {@code 0} to disable.</td>
 * </tr>
 * <tr>
 * <td>{@code allIdleTime}</td>
 * <td>an {@link IdleStateEvent} whose state is {@link IdleState#ALL_IDLE} will be triggered when neither read nor write was performed for the
 ...
 */
public class ChangableIdleStateHandler extends ChannelDuplexHandler {
	...
	private long newReaderIdleTimeNanos = -1;
	private final long newWriterIdleTimeNanos = -1;
	private final long newAllIdleTimeNanos = -1;
	 ...
	 *            an {@link IdleStateEvent} whose state is {@link IdleState#READER_IDLE} will be triggered when no read was performed for the
	 *            specified period of time. Specify {@code 0} to disable.
	 * @param writerIdleTimeSeconds
	 *            an {@link IdleStateEvent} whose state is {@link IdleState#WRITER_IDLE} will be triggered when no write was performed for the
	 *            specified period of time. Specify {@code 0} to disable.
	 * @param allIdleTimeSeconds
	 *            an {@link IdleStateEvent} whose state is {@link IdleState#ALL_IDLE} will be triggered when neither read nor write was performed for
	 *            the specified period of time. Specify {@code 0} to disable.
	 */
	public ChangableIdleStateHandler(int readerIdleTimeSeconds, int writerIdleTimeSeconds, int allIdleTimeSeconds) {
	 ...
	 */
	public ChangableIdleStateHandler(long readerIdleTime, long writerIdleTime, long allIdleTime, TimeUnit unit) {
	...
	public void newReaderIdleTime(long readerIdleTime, TimeUnit unit) {
		switch(state) {
		case 0:
		case 2:
			return;
		...
		if(readerIdleTime <= 0) {
			newReaderIdleTimeNanos = 0;
		}
		else {
			newReaderIdleTimeNanos = Math.max(unit.toNanos(readerIdleTime), MIN_TIMEOUT_NANOS);
		}
	}

	public void newWriterIdleTime(long writerIdleTime, TimeUnit unit) {

	}

	public void newAllIdleTime(long allIdleTime, TimeUnit unit) {
	...
	private final class ReaderIdleTimeoutTask implements Runnable {
		...
		public void run() {
			...
			long lastReadTime = ChangableIdleStateHandler.this.lastReadTime;
			long nextDelay = readerIdleTimeNanos - (currentTime - lastReadTime);

			if(nextDelay <= 0) {
				if(newReaderIdleTimeNanos >= 0) {
					readerIdleTimeNanos = newReaderIdleTimeNanos;
					newReaderIdleTimeNanos = -1;
				}
				...
				else if(newReaderIdleTimeNanos < 0) {
	...
	private final class WriterIdleTimeoutTask implements Runnable {
		...
		public void run() {
			...
			long currentTime = System.nanoTime();
			long lastWriteTime = ChangableIdleStateHandler.this.lastWriteTime;
